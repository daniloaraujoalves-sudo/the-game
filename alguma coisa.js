...
<head>
    <meta charset="UTF-8">
    <title>Mini Terraria - Biomas Avançados</title>
    <style>
        ...
        #minimap-container {
            ...
            border: 2px solid #888; background: rgba(0, 0, 0, 0.8);
...
<div class="info" id="debug-info">Carregando Biomas...</div>
...
const TILE_SIZE = 40;
const WORLD_WIDTH_PX = 40000; // Reduzi um pouco para estabilidade inicial
const WORLD_COLS = Math.floor(WORLD_WIDTH_PX / TILE_SIZE);
const WORLD_ROWS = 90;
const REACH_DISTANCE = TILE_SIZE * 5;

// IDs: 1:Grama, 2:Terra, 3:Pedra, 4:Madeira, 5:Folha, 6:Areia, 7:Água, 8:Ferro, 9:Carvão
const DURABILITY = { 1:15, 2:15, 3:40, 4:25, 5:10, 6:12, 7:999, 8:60, 9:45 };
const tex = {};
...
function initTextures() {
    const colors = {
        ...
        6: ["#f4d03f", "#d4ac0d", "block"],
        7: ["#3498db", "#2980b9", "water"],
        8: ["#dcdde1", "#718093", "block"], // Ferro
        9: ["#2f3640", "#000000", "block"]  // Carvão
    };
...
function getBiome(x) {
    if (x < 100 || x > WORLD_COLS - 100) return "OCEANO";
    const v = Math.sin(x * 0.05);
...
function initWorld() {
    ...
    for (let x = 0; x < WORLD_COLS; x++) {
        ...
        for (let y = 0; y < WORLD_ROWS; y++) {
            if (y < surfaceY) {
                if(biome === "OCEANO" && y > 50) world[y][x] = 7; // Água no oceano
                continue;
            }
            let type = 3; 
            if (y === surfaceY) type = (biome === "DESERTO") ? 6 : (biome === "OCEANO" ? 6 : 1);
            else if (y < surfaceY + 7) type = (biome === "DESERTO") ? 6 : 2;
            
            // Adição de Minérios
            if (y > surfaceY + 15 && Math.random() < 0.03) type = 8;
            if (y > surfaceY + 20 && Math.random() < 0.04) type = 9;
...
function generateTrees() {
    for (let x = 5; x < WORLD_COLS - 5; x += 6) {
        if (getBiome(x) === "FLORESTA" || getBiome(x) === "AMAZONIA") {
            if (Math.random() > 0.3) {
                let groundY = -1;
                for (let y = 0; y < WORLD_ROWS; y++) {
                ...
                if (groundY > 5) {
                    let h = getBiome(x) === "AMAZONIA" ? 8 : 4;
                    for (let i = 1; i <= h; i++) treeWorld[groundY-i][x] = 4;
                    // Folhas simplificadas
                    for(let ly=-2; ly<=0; ly++) 
                        for(let lx=-2; lx<=2; lx++) 
                            if(groundY-h+ly >= 0) treeWorld[groundY-h+ly][x+lx] = 5;
                }
            }
        }
    }
...
const player = { x: WORLD_WIDTH_PX/2, y: 0, w: 20, h: 36, vx: 0, vy: 0, speed: 5, jump: -10, onGround: false };
let inventory = { 1: 10, 2: 10, 3: 0, 4: 0, 6: 0 };
let selectedSlot = 1;
const keys = {};
...
function updateMinimap() {
    minimapTimer++;
    if (minimapTimer % 5 !== 0) return; // Roda a 12fps para economizar CPU
    ...
    const startX = pX - Math.floor(MINIMAP_WIDTH / (2 * MINIMAP_SCALE));
    const startY = pY - Math.floor(MINIMAP_HEIGHT / (2 * MINIMAP_SCALE));

    for (let my = 0; my < MINIMAP_HEIGHT; my++) {
        for (let mx = 0; mx < MINIMAP_WIDTH; mx++) {
            const wx = startX + Math.floor(mx / MINIMAP_SCALE);
            const wy = startY + Math.floor(my / MINIMAP_SCALE);

            if (wx >= 0 && wx < WORLD_COLS && wy >= 0 && wy < WORLD_ROWS) {
                const tile = world[wy][wx];
                const tree = treeWorld[wy][wx];
                if (tree) minimapCtx.fillStyle = tree === 4 ? "#5d4037" : "#2e7d32";
                else if (tile === 1) minimapCtx.fillStyle = "#4caf50";
                else if (tile === 2) minimapCtx.fillStyle = "#795548";
                else if (tile === 3) minimapCtx.fillStyle = "#616161";
                else if (tile === 6) minimapCtx.fillStyle = "#f4d03f";
                else if (tile === 7) minimapCtx.fillStyle = "#3498db";
                else continue;
    ...
    minimapCtx.fillRect(MINIMAP_WIDTH/2 - 2, MINIMAP_HEIGHT/2 - 2, 4, 4);
}
...
function update() {
    ...
    if (world[gy] && Math.hypot(mousePos.x - (player.x + 10), mousePos.y - (player.y + 18)) < REACH_DISTANCE) {
        if (isMouseDown) {
            if (treeWorld[gy][gx]) { treeWorld[gy][gx] = 0; inventory[4] = (inventory[4]||0)+1; updateUI(); }
    ...
    document.getElementById('debug-info').innerText = `Bioma: ${getBiome(Math.floor(player.x/TILE_SIZE))} | Inventário Ativo: ${selectedSlot}`;
...
function draw() {
    ...
    ctx.fillStyle = `rgb(0, ${Math.max(20, 80 + dayRatio * 100)}, ${Math.max(40, 140 + dayRatio * 115)})`;
    ...
    for (let y = Math.max(0, sR); y < Math.min(WORLD_ROWS, eR); y++) {
        for (let x = sC; x < eC; x++) {
